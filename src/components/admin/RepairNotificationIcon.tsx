...
import { Wrench, X, Loader2, MessageSquare } from 'lucide-react';
...
const RepairNotificationIcon: React.FC = () => {
  ...
  const fetchComplaints = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('complaints')
      .select(`id, apartment_number, category, description, status, tenant:profiles(full_name)`)
      .eq('status', 'new')
          ...
          {loading ? (
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {complaints.map((c) => {
                ...
                return (
                  <div key={c.id} className={`flex items-start justify-between p-3 rounded-lg border ${bgColor} ${borderColor}`}>
